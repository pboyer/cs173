...
(define (desugar (exprP : ExprP)) : ExprC
  (type-case ExprP exprP
    ...
    [DeffunP (n ids fbod bod) (LetC n (FuncC (list) (ErrorC (StrC "Dummy function")))
    ...
    [PrimP (op args)
        (case op
          ...
          ['== (cond 
                ((= (length args) 2) (Prim2C '== (desugar (first args)) (desugar (second args))))
                (else (ErrorC (StrC "Bad arguments to =="))))]
          ['print (cond
                    [(= 1 (length args)) (Prim1C 'print (desugar (first args)))]
                    (else (ErrorC (StrC "Only single argument allowed for print"))))]
          ['tagof (cond
                    [(= 1 (length args)) (Prim1C 'tagof (desugar (first args)))]
                    (else (ErrorC (StrC "Only single argument allowed for tagof"))))]
