...
(define (desugar (exprP : ExprP)) : ExprC
  (type-case ExprP exprP
    ...
    ;; in a function, hoist the varP's to the top with an assignment to undefined
    [FuncP (a b) (FuncC a (hoist-desugar b))]
    ...
    [SeqP (e) (build-hoist-expr (hash-keys (foldl hash-merge mt-hoist (map enumerate-vars e))) (seqrec e))]
...
;; find nested var p's, duplicating where necessary
(define (hoist-desugar (e : ExprP)) : ExprC
  (let ((vars (hash-keys (enumerate-vars e))))
    (build-hoist-expr vars (desugar e))))

(define (build-hoist-expr (symbols : (listof symbol)) (inner-expr : ExprC)) : ExprC
  (cond 
    ((empty? symbols) inner-expr)
    (else (LetC (first symbols) (UndefinedC) (build-hoist-expr (rest symbols) inner-expr)))))

(define (hash-merge (a : (hashof symbol boolean)) (b : (hashof symbol boolean))) : (hashof symbol boolean)
  (hash-merge-inner (hash-keys a) b))

(define mt-hoist (hash (list (values '- #t))))

(define (hash-merge-inner (a : (listof symbol)) (b : (hashof symbol boolean))) : (hashof symbol boolean)
  (cond
    ((empty? a) b)
    (else (hash-merge-inner (rest a) (hash-set b (first a) #t)))))

(define (enumerate-vars (e : ExprP)) : (hashof symbol boolean)
  (type-case ExprP e
    
    [TryCatchP (b p c) (hash-merge (enumerate-vars b) (enumerate-vars c))]
    [RaiseP (r) (enumerate-vars r)]
    [VarP (i v) (hash-set (enumerate-vars v) i #t)]
    
    [ObjectP (fs) (foldl 
                   (lambda (x acc) 
                     (type-case FieldP x 
                       (fieldP (n v)
                               (hash-merge (enumerate-vars v) acc))))
                   mt-hoist
                   fs )]
   
    [DotP (o f) (enumerate-vars o)]
    [BracketP (o f) (hash-merge (enumerate-vars o) (enumerate-vars f))]
    [DotMethodP (o f as) (hash-merge (enumerate-vars o) (foldl hash-merge mt-hoist (map enumerate-vars as)))]
    
    [BrackMethodP (o f as) (hash-merge (hash-merge (enumerate-vars o) (enumerate-vars f)) (foldl hash-merge mt-hoist (map enumerate-vars as)))]
    
    ;; in a function, hoist the varP's to the top with an assignment to undefined
    [FuncP (a b) (enumerate-vars b)]
    [AppP (f a) (hash-merge (enumerate-vars f) (foldl hash-merge mt-hoist (map enumerate-vars a)))]
    
    [DefvarP (i bind bod) (hash-merge (enumerate-vars bind) (enumerate-vars bod))]
    [DeffunP (n ids fbod bod) (hash-merge (enumerate-vars fbod) (enumerate-vars bod)) ] 

    [WhileP (test body) (hash-merge (enumerate-vars test) (enumerate-vars body))]

    [ForP (init test update body) (foldl hash-merge mt-hoist (map enumerate-vars (list init test update body)))]

    [AssignP (l r) (type-case LHS l
                     [BracketLHS (o f) (foldl hash-merge mt-hoist (map enumerate-vars (list o f r)))]
                     [DotLHS (o f) (foldl hash-merge mt-hoist (map enumerate-vars (list o r)))]
                     [IdLHS (i) (enumerate-vars r)])]
    
    [IfP (c t e) (foldl hash-merge mt-hoist (map enumerate-vars (list c t e)))]
    [SeqP (e) (foldl hash-merge mt-hoist (map enumerate-vars e))]
    
    [PrimP (op args) (foldl hash-merge mt-hoist (map enumerate-vars args))]
    
    [PrimAssignP (o l v) (type-case LHS l
                           [BracketLHS (ob f) (foldl hash-merge mt-hoist (map enumerate-vars (list ob f v)))]
                           [DotLHS (ob fs) (foldl hash-merge mt-hoist (map enumerate-vars (list ob v)))]
                           [IdLHS (i) (enumerate-vars v)])]
